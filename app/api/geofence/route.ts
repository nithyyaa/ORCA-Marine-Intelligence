...
function mapZone(row: BoundaryRow) {
  ...
  const distanceMeters = toNumberOrNull(row.distance_to_boundary_meters);
  const warning = warningForZone(inside, distanceMeters, type);
...
export async function GET(request: Request) {
  try {
    ...
    const typeResult = await db.execute(sql`
      SELECT DISTINCT UPPER(TRIM(boundary_type)) AS boundary_type
    ...
    const availableZoneTypes = Array.from(typeResult as any)
      .map((row: any) => normalizeZoneType(row.boundary_type))
      .filter(Boolean);

    const unavailableZoneTypes = SUPPORTED_ZONE_TYPES.filter(
      (type) => !availableZoneTypes.includes(type)
    );
        ...
        ST_AsGeoJSON(
          CASE
            WHEN ${url.searchParams.get("geometry")} = 'full'
              THEN geometry
            ELSE ST_SimplifyPreserveTopology(geometry, 0.01)
          END
        ) AS geometry_geojson
    ...
    let rows = Array.from(insideResult as any) as BoundaryRow[];
    ...
    if (rows.length === 0) {
      ...
      rows = Array.from(candidateResult as any) as BoundaryRow[];
    }
    ...
    const zones = rows
      ...
      .sort(
        (a, b) =>
          (a.distanceToBoundaryMeters ?? Number.POSITIVE_INFINITY) -
          (b.distanceToBoundaryMeters ?? Number.POSITIVE_INFINITY)
      );

    const insideZones = zones.filter((zone) => zone.inside);

    const approachingZones = zones.filter(
      (zone) => zone.status === "APPROACHING"
    );

    const violationZones = zones.filter((zone) => zone.violation);

    /*
     * Keep one nearest relevant zone per type in the compact
     * response. This prevents returning dozens of large geometries.
     */
    const nearestByType = new Map<string, (typeof zones)[number]>();
    ...
    const relevantZones = Array.from(nearestByType.values());

    // Legacy EEZ compatibility.
    const eezZones = zones.filter((zone) => zone.type === "EEZ");
    const nearestEEZ = eezZones[0] ?? null;
    const insideEEZ = eezZones.some((zone) => zone.inside);
    ...
    const distanceToBoundaryKm =
      distanceToBoundaryMeters !== null
        ? Number((distanceToBoundaryMeters / 1000).toFixed(2))
        : null;

    let warning = "NO_BOUNDARY_DATA";

    if (nearestEEZ) {
      warning = insideEEZ ? nearestEEZ.warning : "OUTSIDE_EEZ";
    }
    ...
    return NextResponse.json({
      ...
      geometryMode,
      geofenceStatus,
      violationCount: violationZones.length,
      approachingCount: approachingZones.length,

      geofencing: {
        engine: "PostGIS Unified Geofence Engine",
        prototype: true,
        ...
        zoneTypeAvailability: {
          EEZ: availableZoneTypes.includes("EEZ"),
          RESTRICTED: availableZoneTypes.includes("RESTRICTED"),
          MPA: availableZoneTypes.includes("MPA"),
          ECOLOGICALLY_SENSITIVE:
            availableZoneTypes.includes("ECOLOGICALLY_SENSITIVE"),
          OPERATIONAL: availableZoneTypes.includes("OPERATIONAL"),
          HAZARD: availableZoneTypes.includes("HAZARD"),
        },
