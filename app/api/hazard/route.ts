  ...
  for (const feed of feeds) {
    try {
      ...
      for (const item of items) {
        ...
        const text =
          `${title} ${description} ${area} ${event}`.toLowerCase();

        if (
          !/(cyclone|cyclonic storm|deep depression|depression)/i.test(
            text
          )
        ) {
          continue;
        ...
        if (
          area &&
          !locationMatches(area, location) &&
          !/india|nationwide|all india/i.test(area)
        ) {
          continue;
        }

        const severe =
          /extremely severe|very severe|super cyclonic|severe cyclonic/i.test(
            text
          );
        ...
        hazards.push({
          ...
          title: severe
            ...
            : "Cyclone Warning",
          message: `${
            title || event || "Cyclone-related official alert"
          }${area ? ` Area: ${area}.` : "."}`,
          source: feed.source,
...
async function fetchDirectWeather(
  ...
  try {
    ...
    const currentPrecipitation = num(
    ...
    const hourlyPrecipitation = Array.isArray(
    ...
    const hourlyWeatherCode = Array.isArray(
    ...
    return {
      precipitation:
        currentPrecipitation ?? hourlyPrecipitation,
      weatherCode:
        currentWeatherCode ?? hourlyWeatherCode,
      available: true,
    };
  } catch {
    return {
...
async function fetchDirectMarine(
  latitude: number,
  longitude: number
): Promise<{
  ...
  try {
    ...
    const currentWave = num(
      data?.current?.wave_height
    );

    const hourlyWave = Array.isArray(
...
export async function GET(request: Request) {
  ...
  try {
    ...
    /*
     * Prefer the actual observation timestamp returned
     * by the upstream Weather/Ocean APIs.
     * Fall back to the current server time only when
     * the upstream timestamp is unavailable.
     */
    const observedAt =
      weather?.time ??
      ocean?.time ??
    ...
    const waveHeight =
      num(ocean?.waveHeight) ??
      directMarine.waveHeight;
    ...
    if (
      directWeather.weatherCode !== null &&
      directWeather.weatherCode >= 95
    ) {
      hazards.push({
        type: "THUNDERSTORM",
        severity:
          directWeather.weatherCode >= 99
            ? "HIGH"
            : "MODERATE",
        title: "Thunderstorm Hazard",
        message:
          "Thunderstorm conditions are indicated by the current weather model.",
    ...
    if (
      (waveHeight !== null && waveHeight >= 2) ||
      (windSpeed !== null && windSpeed >= 30)
    ) {
      hazards.push({
        type: "HAZARDOUS_SEA",
        severity:
          (waveHeight !== null && waveHeight >= 4) ||
          (windSpeed !== null && windSpeed >= 45)
            ? "CRITICAL"
            : "HIGH",
        title: "Hazardous Sea Conditions",
        message:
          "Combined wind and wave conditions may create unsafe marine conditions.",
        source:
    ...
    return NextResponse.json({
      ...
      sources: {
        ...
        ocean:
          ocean.source ??
          (directMarine.available
            ? "Open-Meteo Marine API (direct fallback)"
        ...
        cyclone:
          cyclone.source ??
          (cyclone.available
            ? "NDMA SACHET CAP / IMD alert feed"
            : null),
    ...
    return NextResponse.json(
      {
        ...
        hazards: [],
        error:
          "Unable to generate marine alerts",
      },
      { status: 500 }
    );
  }
