...
function MarineMapLoader() {
  const [MarineMapComponent, setMarineMapComponent] = useState<ComponentType | null>(null);
  ...
  useEffect(() => {
    ...
    import("@/components/MarineMap")
      .then((mod) => {
        const component = (mod as any).default ?? (mod as any).MarineMap;
        if (mounted && typeof component === "function") {
          setMarineMapComponent(() => component);
        }
      })
      .catch((error) => {
        console.error("Failed to load MarineMap:", error);
      });
  ...
  if (!MarineMapComponent) {
    return (
      <div className="flex h-full min-h-[500px] items-center justify-center rounded-2xl border border-white/10 bg-[#071525] text-slate-400">
        Loading marine map…
      </div>
    );
  }

  return <MarineMapComponent />;
}
...
function DataFreshness({ data }: { data: DashboardData | null }) {
  ...
  const items = [
    {
      name: "Weather",
      item: freshness?.weather,
    },
    {
      name: "Ocean",
      item: freshness?.ocean,
    },
    {
      name: "Tide",
      item: freshness?.tide,
  ...
  const statusClass = (status: string | undefined) => {
    ...
    if (status === "AGING") return "caution";
    return "red";
  };
      ...
      {items.map(({ name, item }) => (
        <span className="source" key={name}>
          ...
          item?.ageMinutes !== undefined
            ? ` · ${Math.round(item.ageMinutes)} min ago`
            : ""}
        </span>
      ))}
    </div>
